...
- Stateless JWT auth：登入成功時 `AuthLoginResponse.tokenPair` 回傳 `{ accessToken, refreshToken }`，**同時**後端也把兩個 token 寫進 **HttpOnly + Secure + SameSite=None** cookie（見 `AuthServiceImpl.buildTokenCookie`）。`Secure` 由 `.env` 的 `COOKIE_SECURE` 控制（`application.properties` 的 `cookie.secure=${COOKIE_SECURE:true}`，未設預設 true；本機純 http 開發設 false）。`JwtAuthenticationFilter` 讀取 token 時優先用 `Authorization: Bearer <accessToken>` header，找不到才 fallback 讀 cookie。
